# 📈 ASFM: 대규모 LLM 기반 가상 금융 시장 시뮬레이터

> **"500명의 AI가 만들어내는 가장 현실적인 주식 시장, 그리고 완벽한 워크플로우 최적화"**
> 
> 실제 인간의 투자 심리(행동경제학)를 74% 수준으로 모사하는 500명의 생성형 에이전트들이 활동하는 가상 경제 생태계입니다. 실제 주식 시장과 동일한 호가 매칭 엔진을 백엔드에 구현하였으며, 특히 **메타 휴리스틱 알고리즘을 도입해 LLM API 운영 비용을 49.5% 절감**한 상용화 수준의 대규모 AI 시뮬레이터입니다.

---

## 💡 주요 기능 (Core Features)

### 1. 500명의 생성형 에이전트 사회 (Agent Society)
...
- **해결:** 유저가 정상 호가(±10%) 내에서 주문 시, **Market Maker(시장 조성자) 알고리즘**이 개입하여 즉각적으로 유동성을 공급하는 하이패스 로직을 구현했습니다.
